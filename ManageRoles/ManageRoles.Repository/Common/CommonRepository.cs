...
namespace ManageRoles.Repository
{
    ...
    public class CommonRepository<T> : IDisposable where T : class
    {
        ...
        public void setModel(StringBuilder searchQ, CommonModel<T> model, Dictionary<string, string> colName, string sortFieldName, int page = 1, int pageSize = 10)
        {
            ...
            foreach (var item in colName)
            {
                if (model.Filters != null)
                {
                    ...
                    if (obj == null)
                    {
                        obj = new GridFilter { ColumnName = item.Key, DisplayText = item.Value };
        ...
        public void setModel(string searchQ, CommonModel<T> model, Dictionary<string, string> colName, int page = 1, int pageSize = 10)
        {
            model.Filters = model.Filters == null ? new List<GridFilter>() : model.Filters;
